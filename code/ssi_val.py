...
def arg_parser():
    parser = argparse.ArgumentParser(description='ssi_args')
...
def SSI(x):
    SSI = np.zeros((x.shape[0], 1))
    for k in range(x.shape[0]):
        SSI[k] = np.sum(np.square(x[k, :]))
    return SSI
...
model_name = 'ssi' + 'p' + str(args.prediction)
print(str(tt_set)+'\n', str(batch_size)+'\n', str(num_layers)+'\n', str(hidden_size)+'\n',
...
X_test_SSI = (X_test_SSI-0.3957438647053795)/0.9240087261550427 # z-score norm with mean and sigma same as training set.
x_te = X_test
y_te = X_test_SSI
...
np.savetxt('SSI0_R2'+str(args.prediction)+'.txt', my_array, fmt='%.4f')
...
plt.title("R2 SSI Distribution")
plt.xlabel("Accuracy")
plt.ylabel("Frequency")
plt.savefig("ssi0_R2"+str(args.prediction)+".png")